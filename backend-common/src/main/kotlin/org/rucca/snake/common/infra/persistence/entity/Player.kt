...
@Entity
@Table(name = "players", indexes = [Index(name = "idx_players_user_id", columnList = "user_id")])
data class Player(
    @Id
    // No @GeneratedValue if user_id is the PK and comes from User
    @Column(name = "user_id", nullable = false)
    var userId: Int = 0, // Corresponds to User.id (which is Int?)
    @Column(nullable = false, length = 255) var nickname: String = "", // Denormalized nickname
    ...
    @Column(
        ...
        nullable = false,
        updatable = false,
    ) // Typically only set on creation
